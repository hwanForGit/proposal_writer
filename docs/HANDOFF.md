...
- **사업비 계산 탭 (신규 기능 전체)** — 제안서 작성과 별개. 산출 후 복사해서 계획서에 붙여넣는 용도.
  - **상태 store 2개**: `src/features/labor/store.ts`(용역비/인건비, persist `proposal_writer.labor.v1` v5), `src/features/projectCost/store.ts`(총사업비, persist `proposal_writer.projectcost.v1`). 클립보드: `src/lib/clipboard.ts`(비-secure context 폴백).
  ...
  - **인력 행**: 성명/역할, 연봉(or 월단가), 참여개월, 투입률(고정값 또는 **범위=최대 상한선만**), **출처 드롭다운**(현물/정부/현금, 수동 수정 가능).
  - **⚙️ 자동 계산하기** 버튼(`autoCalculate`): 고정 투입률 유지 / 변동은 **최대 적용** / 출처를 **현물→정부→현금 순 best-fit(FFD, 비용 큰 사람부터, 한 명=한 출처)** 자동 배정 / 경계 인원은 **참여개월을 1~사업기간 내에서 줄여 예산에 맞춤** / 예산 소진 후 미배정 인원은 0개월(산출 0).
  - **결과 복사**: Markdown 표 / 엑셀(탭 구분).
  - 참고 문서: `docs/[별표 2]…`, `docs/별첨3…` PDF (국가연구개발사업 비목별 계상기준 — 인건비 산식 근거).
  - **미해결/다음 후보**: 출처 best-fit는 FFD 휴리스틱이라 정수 개월 단위로 마지막 출처에 소액 잔여 가능(전역 최적 아님). 연구수당/간접비 비목은 보류. 출처 배정·참여개월 정밀화 여지.
