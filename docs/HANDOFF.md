...
### 2026-06-16 세션 (방금 작업 — commit/push 완료) ★최신
1. **프롬프트 다듬기 (공고·양식 100% 준수 + 모르면 표기)** — `prompts/*.md` 런타임 파일 직접 수정(서버가 매 호출 read → 즉시 반영). `prd_*.md`(설계 원본)는 미수정.
   - **공고·양식 100% 적용**: `outline_step1`에 새 출력 섹션 **「✅ 공고·양식 준수 필수사항」**(필수 기재 항목·분량/서식·작성 절차·평가 배점 추출) 추가 — 본문(`body_phase2`)은 공고/양식 원문을 안 받으므로 이 체크리스트가 `step1Markdown`을 타고 Step2·본문·페이지배분까지 전달돼 규정이 끝까지 닿음(코드 변경 없이). `step2_sections`/`step2_section`/`body_phase2`도 목차뿐 아니라 작성 절차·서식·분량까지 준수 + Step1 체크리스트 반영.
   - **모르면 지어내지 말 것**: 통일 마커 **`🔲 [확인 필요: ...]`** 도입. 웹 검색 미연동 현실 반영 — `outline_step1`·`body_phase2`의 "실시간 검색" 과장을 "확신하는 지식만, 모르면 마커 표기, 가짜 수치·출처 금지"로 교체. `body_phase2` 부록을 "출처 / 확인 필요 리스트"로 변경.
   - **순서 절대 보존**: `step2_section`에 ★★ 블록 추가(양식 등장 순서 위→아래 그대로, 재배열/통합/분리 금지). `step2_sections`도 대분류 순서 보존 명시. ※ 사용자 보고: "2. 제안 개요"에서 중분류 순서가 양식과 달랐음 → 프롬프트는 best-effort라 아래 ②로 보완.
2. **중분류별 페이지 배분 커스텀** (`workspace/store.ts` + `OutlinePanel.tsx`) — AI 자동 배분 외에 수동 조정 추가. `AllocationItem.manual?` 플래그, `setAllocationPages(key,pages)`(0.5단위 덮어쓰기), `initManualAllocation()`(LLM 없이 균등 분배 시작). 패널에 **`직접 입력` 버튼** + 합계 vs 목표 색상 인디케이터(일치=녹/초과=빨/부족=앰버) + 접이식 **「중분류별 페이지 직접 조정」**(대분류 그룹·0.5단위 입력). 편집값이 트리 배지·본문 목표 자수에 즉시 반영. 헬퍼 `collectMidItems`/`evenSplitPages` 추출.
3. **가이드 숫자 깨짐 수정** (`SectionTreeView.tsx` `splitItemized`) — 재무현황/유사실적 가이드가 "2. 2", "3. 0"처럼 깨지던 버그. 원인 2개: (a) 마커 스트립 `^\d+[.)]\s*`이 소수 정수부까지 삭제(`2.2조`→`2조`, `2.2`→`2`) + `<ol>` 재번호 → "2. 2"로 보임, (b) 임의 `숫자.공백`을 마커로 오인해 과분할. 수정: **문장 시작/공백 뒤 + 1,2,3… 연속 수열**일 때만 리스트 인정, 소수(마커 뒤 공백 없음)는 마커 아님, 검증된 마커만 정확히 제거. **렌더 단계 버그라 재생성 불필요 — 새로고침이면 기존 데이터도 정상 표시.**
4. **순서 변경 ↑/↓ 버튼** (`SectionTreeView.tsx`) — 트리 편집기에 중분류·소분류 순서 변경(인접 swap→재직렬화→저장). 경계 자동 비활성화. `ReorderButtons` 컴포넌트. LLM이 순서 틀려도 확실히 교정 가능(프롬프트 ①의 보완책).
5. **`server/exporters/pdf.ts` 타입에러 정리** — `setContent` `waitUntil:'networkidle0'`(puppeteer 타입 미허용) → 인라인 스타일 자기완결이라 `'load'`로 교체. `cd server && npm run typecheck` 통과.

### ⚠️ 다음 세션 우선 (다른 컴에서 이어가기)
- **(미검증) 본문 페이지 수 캘리브레이션** — 직전 세션 미해결 그대로. 코드 분석 결론: 90p 초과 원인은 (a) **페이지 배분 미적용**(목표자수 null → 모든 중분류가 풀 분량 2,500~3,500자). `proceedToStep3` 자동 배분 생성으로 해결됐을 것이고 `CHARS_PER_PAGE=1300`은 대체로 맞는 값(밀도 불일치 아님). **검증: 목표 페이지 설정 → 본문 재생성 → '본문만' 다운로드 → 페이지 수 확인.** 어긋나면 `CHARS_PER_PAGE`(workspace/store.ts:180) 소폭 보정. ※ 함정: 자동 배분은 `pageLimit` 설정됐을 때만 작동.
- **(검증 요청) 프롬프트 효과** — 새 머신에서 Step1~본문 한 번 돌려 ① 공고/양식 규정 반영 ② `🔲 [확인 필요]` 마커가 가짜 데이터 대신 잘 나오는지 ③ 중분류 순서 보존 확인.
...
- ~~서버 `exporters/pdf.ts`의 `waitUntil:'networkidle0'` 타입에러~~ → **2026-06-16 해결**(`'load'`로 교체).
